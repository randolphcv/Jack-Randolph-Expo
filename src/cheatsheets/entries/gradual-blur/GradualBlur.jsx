...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (width <= 480 && config[`mobile${responsiveKey}`]) {
        nextValue = config[`mobile${responsiveKey}`];
      } else if (width <= 768 && config[`tablet${responsiveKey}`]) {
        nextValue = config[`tablet${responsiveKey}`];
      } else if (width <= 1024 && config[`desktop${responsiveKey}`]) {
        nextValue = config[`desktop${responsiveKey}`];
      }
...
function GradualBlur(props) {
  ...
  useEffect(() => {
    if (isVisible && animated === 'scroll' && onAnimationComplete) {
      ...
      const timeoutId = setTimeout(() => onAnimationComplete(), milliseconds);
